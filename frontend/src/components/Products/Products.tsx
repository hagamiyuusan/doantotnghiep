    ...
    product.durations.map((duration) => (
      <div>
        <div className='grid-cols-1 bg-white rounded shadow-sm pt-4 ' style={{ margin: '0px 10px' }}>
          ...
          <div className=' px-4'>
            <button
              onClick={profile?.userName ? () => payload(duration.original_duration_id, profile?.userName) : () => setShowModalLogin(true)}
              type='button'
              className='w-full text-gray-900 bg-[#F7BE38] hover:bg-[#F7BE38]/90 focus:ring-4 focus:outline-none focus:ring-[#F7BE38]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#F7BE38]/50 mr-2 mb-2'
            >
              <svg
