...
export default function DuarationManager() {
  ...
  const handleOkeDelete = async () => {
    try {
      const res = await axios.delete(`${import.meta.env.VITE_BASE_URL}/Duration/${duration.id}`, {
        headers: {
          ...
          <tbody>
            {durations.map((duration, index) => (
              <tr className='bg-white border-b dark:bg-gray-800 dark:border-gray-700' key={duration.id}>
                ...
                <td className='px-6 py-4'>{duration.name}</td>
                <td className='px-6 py-4'>{duration.day}</td>
